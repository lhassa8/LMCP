...
from .types import (
    ToolResult, ResourceResult, PromptResult, ServerInfo, ToolInfo, 
    ResourceInfo, PromptInfo, ConnectionConfig, HealthStatus
)
from .exceptions import (
    LMCPError, ConnectionError, ServerError, ValidationError,
...
class Client:
    ...
    async def call_tool(self, name: str, *args, **kwargs) -> ToolResult:
        ...
        try:
            logger.debug(f"Calling tool '{name}' with args={args}, kwargs={kwargs}")
            result = await self._mcp_client.call_tool(name, *args, **kwargs)
    ...
    async def get_prompt(self, name: str, arguments: Optional[Dict[str, Any]] = None) -> PromptResult:
